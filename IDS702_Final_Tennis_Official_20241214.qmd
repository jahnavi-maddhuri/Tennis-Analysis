...
In regard to the first research question, Multiple linear regression (MLR) was used to analyze the factors influencing match duration. Variance Inflation Factor (VIF) was calculated to test for multicollinearity among predictors, and all variables showed acceptable levels of collinearity, including the interaction terms. Assumptions of linear regression, such as normality, homoscedasticity, and linearity, were evaluated using diagnostic plots. To address deviations from normality, a log transformation was applied to the target variable, match duration, which significantly improved the distribution of residuals. Residual versus fitted plots and Q-Q plots were used to assess model assumptions and ensure the validity of results. R-squared was used to evaluate the overall performance of the linear regression model. All statistical analyses were performed using R programming language (version 4.3.1)
...
Also, the court surface plays a pivotal role in match duration. Matches on clay courts last significantly longer, with durations increasing by 14% (95% CI: 10.1%, 17.8%, p < 0.001). However other types of surfaces have shown no significant impact on length of the match.

Finally, the heights of both winners and losers contribute to match duration. For each additional unit of height for the winner, match duration increases by 0.3% (95% CI: 0.2%, 0.3%, p < 0.001). Conversely, loser height has a smaller negative impact, decreasing match duration by 0.1% (95% CI: -0.1%, -0.06%, p = 0.006). This suggests that taller winners may leverage their physical advantage for strategic play, while loser height appears to have a negligible effect on reducing match duration.

The model achieved an R-squared of 0.31.
...
formatted_results <- tidy_model %>%
  ...
  dplyr::select(term, estimate, std.error, Lower_CI, Upper_CI, p.value) %>%
  rename(
    Predictors = term,
...
formatted_results %>%
  knitr::kable(
    caption = "Linear Regression Model Results with Manually Computed Confidence Intervals",
    digits = 3,
    align = "c",
    format = "markdown"
  )
...
```{r, warning=FALSE, echo=FALSE, include=FALSE, results = 'hide'}
# Calculate VIF for predictors to determine multicolinearity
library(car)

# Extract the first imputed dataset for simplicity (or one representative dataset)
final_data <- complete(tennis.imp, 1)

# Refit the model for VIF calculation
vif_model <- lm(log(minutes) ~ diff_rank * tourney_level + diff_aces + 
                winner_ht + loser_ht + surface + winner_age + loser_age + 
                w_df + l_df + w_bpSave_ratio + l_bpSave_ratio, 
                data = final_data)
...
# Scatterplot between diff_rank and minutes, separated by tourney_level

library(mice)

# Extract completed data frame from mice object
tennis_imp_df <- complete(tennis.imp)  # Choose the first imputed dataset
...
# Combine confidence intervals into a single column and add the standard error
coeff_df <- data.frame(
...
colnames(coeff_df) <- c("Variable", "Coefficient","Standard Error", "Odds Ratio", "95% CI", "P-value")

# Create the table and align all columns to the right
kable(coeff_df, caption = "Logistic Regression Coefficients, Odds Ratios, Confidence Intervals, Standard Errors, and P-values", 
      align = c("l", "r", "r", "r", "r", "r")) %>%  # Align first column to the left and others to the right
  kable_styling(full_width = FALSE)

```

Analyzing the variables of interest aces and surface, we have an effect of 1.12 times increase in odds of winning for every extra ace point when the match is disputed on a clay surface, with a combined confidence interval between 1.03 and 1.19, considering that the variable aces is statistically significant while the interation terms are not. Similarly, for matches played on Grass the model has a combined effect increasing the odds of winning by 1.08 times. The impact of every extra ace point while playing on hard surfaces represent a combine effect of 1.09 increse in the odds of winning the match. Another statistically significant variable impacting the odds of winning is the right vs left-handed stature of the player. Specifically being Right or Left Handed (as opposed to ambidextrous) is associated with approximately 1.35 times increase in odds of winning. Furthermore, the break point serve ration explains an effect of 1.213 times the increase in odds of winning.
...
## Conclusion
This study identified key factors influencing tennis match durations, shedding light on how player characteristics, tournament conditions, and performance metrics contribute to match length. Notably, tournament levels emerged as a significant determinant, with higher-level tournaments, such as G and M, associated with substantially longer matches, reflecting greater competitiveness and intensity. Other significant predictors included differences in player performance, such as the number of aces and breakpoints saved, both of which revealed nuanced relationships with match duration. Physical characteristics, such as player height, also contributed, albeit modestly, suggesting a strategic role in gameplay. 

Despite these insights, the study faced limitations. The reliance on imputed data may have introduced bias, while the linear regression model's assumptions, such as normality, were not perfectly met despite transformations. Plus, the model explained 31% of the variance in match duration (R² = 0.31), leaving room for unaccounted influences such as weather, fatigue or external conditions, potentially leaving room for unexplored contributors to match durations. These limitations may have influenced the precision of the model's estimates
