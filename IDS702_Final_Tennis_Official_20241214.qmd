...
imp.mods <- with(tennis.imp, lm(minutes ~ diff_rank + diff_aces + winner_ht + loser_ht + surface + 
            tourney_level + winner_age + loser_age + w_df + l_df + 
            w_bpSaved + l_bpSaved + w_bpFaced + l_bpFaced))
summary(pool(imp.mods))

# Original without inputation:
set.seed(123)
model_linear_2 <- lm(minutes ~ diff_rank + diff_aces + winner_ht + loser_ht + surface + 
            tourney_level + winner_age + loser_age + w_df + l_df + 
            w_bpSaved + l_bpSaved + w_bpFaced + l_bpFaced, data = tennis)
